...
hadoop fs -du -h $SOURCE_DIR >> $LOG_FILE_NAME 2>/dev/null

hadoop fs -du -s -h $SOURCE_DIR >> $LOG_FILE_NAME 2>/dev/null

hadoop fs -mkdir $TMP_DIR >> $LOG_FILE_NAME 2>/dev/null
...
hadoop fs -rm -r $TMP_DIR >> $LOG_FILE_NAME 2>/dev/null

echo "$TMP_DIR is removed" >> $LOG_FILE_NAME

echo "Size of dir after compression ================================================" >> $LOG_FILE_NAME

hadoop fs -du -h $TARGET_DIR >> $LOG_FILE_NAME 2>/dev/null

hadoop fs -du -s -h $TARGET_DIR >> $LOG_FILE_NAME 2>/dev/null
