...
object GeneratorOptions {
    private val parser: OptionParser[GeneratorOptions] = new scopt.OptionParser[GeneratorOptions]("Generator options") {
        ...
        opt[String]("requestType").action((x, c) => c.copy(requestType = x)).text("Request type (APPEND|DELETE|REPLACE)")
        opt[String]("partitionSpec").action((x, c) => c.copy(partitionSpec = x)).text("RegExp for partitions to match with")
